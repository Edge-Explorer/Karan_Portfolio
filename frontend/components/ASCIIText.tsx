...
class AsciiFilter {
  ...
  asciify(ctx: CanvasRenderingContext2D, w: number, h: number) {
    try {
      ...
      // Ignored in case canvas is empty during resize
    }
  }
...
class CanvasTxt {
  ...
  resize() {
    ...
    const textHeight = Math.ceil(metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent || this.fontSize) + 30;

    this.canvas.width = Math.max(10, textWidth);
    this.canvas.height = Math.max(10, textHeight);
  }
  ...
  render() {
    ...
    const metrics = this.context.measureText(this.txt);
    const yPos = 15 + (metrics.actualBoundingBoxAscent || this.fontSize * 0.8);
...
class CanvAscii {
  ...
  updateRotation() {
    if (!this.mesh) return;
    const x = mapRange(this.mouse.y, 0, this.height, 0.4, -0.4);
    const y = mapRange(this.mouse.x, 0, this.width, -0.4, 0.4);
